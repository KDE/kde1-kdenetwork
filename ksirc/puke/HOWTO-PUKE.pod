...
is done via a fixed length 64 byte message with the following layout:
...
None of the fields except for iCommand have any restrictions on their
content and may contain arbitrary values.  iCommand must contain an
int and it used by ksirc to determine the destination/handler of the actual
command. (and of course it's meaning).

=item Internal handling by kSirc:

Messages are received by a generic handler, PukeController where based 
on ranges for iCommand they are shipped out to be handle by a specific 
handler.  For example range 1000-10000 is assigned to widget
commands.  All commands in that range are sent to the WidgetRunner
which then interprets them as required.  Internall all messages are
messages are transfered with the additional int descriptor fd.  Fd it
used to identify the various client connections.
