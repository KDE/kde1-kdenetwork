...
/** KProgress is derived from QFrame
  ...
  signal to it. */
class Progress : public QFrame, public QRangeControl {
	Q_OBJECT
public:
  ...
  /** Construct a KProgress bar with an orientation. */
  Progress(Orientation, QWidget *parent=0, const char *name=0);
  ...
  /** Construct a KProgress bar with minimum, maximum and initial
	value. */
	Progress(int minValue, int maxValue, int value, Orientation, 
